...
/***/ function(module, exports, __webpack_require__) {
	...
	var _AllCards = __webpack_require__(4);

	var _AllCards2 = _interopRequireDefault(_AllCards);
	...
	var page = new _Page2.default({
		error: null,
		allCards: new _AllCards2.default()
	});

	var pageView = new _PageView2.default({
		el: document.body,
		model: page
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var AllCards = Backbone.Collection.extend({
		...
		loadCard: function loadCard(model) {
			var _this2 = this;

			var url = model.get('fetchUrl');
			...
			fetch(url).then(function (response) {
				...
				var commits = json.slice(0, 5).map(function (obj) {
					return {
				...
				_this2.trigger('error', { message: error.message, timestamp: Date.now() });
			});
		},

		deleteCard: function deleteCard(model) {
			this.remove(model);
		},

		addRepo: function addRepo(title) {
			var url = 'https://api.github.com/repos/' + title + '/commits?client_id=' + _config.client.id + '&client_secret=' + _config.client.sec;

			var isRepeat = this.findWhere({ repoTitle: title.toLowerCase() });
			...
			this.loadCard(new _Card2.default({
				justAdded: true,
...
/***/ function(module, exports) {
	...
	var client = exports.client = {
		id: 'bb5c6137c1221ebe961e',
		sec: '8e3e986c7b1761baef0161279fc23a69d7be9449'
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _AllCardsView = __webpack_require__(9);

	var _AllCardsView2 = _interopRequireDefault(_AllCardsView);

	var _RightMenuView = __webpack_require__(11);

	var _RightMenuView2 = _interopRequireDefault(_RightMenuView);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var PageView = Backbone.View.extend({
		render: function render() {
			var allCards = this.model.attributes.allCards;
			...
			var header = new _SearchView2.default({
				model: this.model,
				el: headerDiv
			}).render();

			var rightMenu = new _RightMenuView2.default({
				collection: allCards,
				el: rightMenuDiv
			}).render();

			var container = new _AllCardsView2.default({
				collection: allCards,
				el: containerDiv
			}).render();
...
/***/ function(module, exports) {
	...
	var SearchView = Backbone.View.extend({
		...
		render: function render() {
			...
			this.el.innerHTML = '\n\t\t\t<form class="repo-form">\n\t\t\t\thttps://github.com/\n\t\t\t\t<input type="text" name="repository" size="30" autofocus>\n\t\t\t\t<button type="submit" class="btn add-btn">Add</button>\n\t\t\t\t' + (this.model.get('error') ? '<span class="err-msg invisible">' + this.model.get('error').message + '</span>' : '') + '\n\t\t\t</form>\n\t\t';

			setTimeout(function () {
				var errMsg = _this.el.querySelector('.err-msg');
		...
		addRepo: function addRepo(event) {
			event.preventDefault();

			var title = this.el.querySelector('[name="repository"]').value;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _CardView = __webpack_require__(10);
	...
	var AllCardsView = Backbone.View.extend({
		...
		render: function render() {
			...
			var cardsToShow = void 0;

			if (this.collection.mode === 'showStarred') {
				cardsToShow = this.collection.models.filter(function (card) {
					return card.get('starred');
				}) || [];
			} else {
				cardsToShow = this.collection.models;
			}

			cardsToShow.forEach(function (card, i) {
				if (card.get('justAdded') === true) {
					var cardView = new _CardView2.default({ model: card, className: 'card card-invisible' });
					_this.collection.get(card).set('justAdded', false);
				} else {
					var cardView = new _CardView2.default({ model: card });
				}

				_this.el.insertBefore(cardView.render().el, _this.el.firstChild);
				_this.collection.syncStorage();
			});
...
/***/ function(module, exports) {
	...
	var CardView = Backbone.View.extend({
		...
		render: function render() {
			...
			var commitsListHtml = this.model.get('commits').reduce(function (string, commit, i) {
			...
			this.model.get('starred') ? this.el.classList.add('starred') : this.el.classList.remove('starred');

			setTimeout(function () {
				_this.el.classList.remove('card-invisible');
			}, 100);
		...
		delete: function _delete() {
			...
			setTimeout(function () {
				_this2.model.collection.deleteCard(_this2.model);
			}, 1000);
		},

		star: function star() {
			this.model.get('starred') ? this.model.set('starred', false) : this.model.set('starred', true);
	...
	var RightMenuView = Backbone.View.extend({
		...
		refreshAll: function refreshAll() {
			...
			this.collection.forEach(function (card) {
				_this.collection.loadCard(card);
			});
...
/***/ function(module, exports) {
	...
	var Page = Backbone.Model.extend({
		initialize: function initialize() {
			...
			this.listenTo(this.get('allCards'), 'error', function (error) {
